# 👋Hi! I'm José Luiz ( call me Junior 😅)

- 👨🏻‍💻 I started working in development in 2010.
- 💻 My main stack is Java, but I've worked with Scala, Kotlin, Node, Python and C#.
- 👀 I've worked with frontend (Fullstack) using Angular and React and worked with mobile (Xamarin.Forms).
- 💿 Strong knowledge of SQL databases and I've worked with NoSQL (MongoDB, DynamoDB).
- 🧐 I'm curious about new stacks, have some projects in various stacks ( ex.: Groovy, React Native, Vue, etc).

## 👨🏻‍🎓 Formations
 - **Graduation:** FATEC Guaratinguetá - Computer Technology with Emphasis on Database, August 2009 - August 2012 .
 - **Post Graduation:** FATEA - Specialization in Design and Development of Web Applications, March 2014 - September 2015.
 - **Post Graduation:** FIAP - A.I. For Developers, March 2025 - March 2026.

## 💼 Companies where I already worked:
<p align="left">
  <img src="https://github.com/juniorjrjl/juniorjrjl/blob/main/assets/logos/embraer.png" alt="Embraer" width="50" height="50"/>
  <img src="https://github.com/juniorjrjl/juniorjrjl/blob/main/assets/logos/picpay.jpeg" alt="PicPay" width="50" height="50"/>
  <img src="https://github.com/juniorjrjl/juniorjrjl/blob/main/assets/logos/caju.png" alt="Caju" width="50" height="50"/>
  <img src="https://github.com/juniorjrjl/juniorjrjl/blob/main/assets/logos/rd-drogasil.png" alt="Grupo RD Drogasil" width="50" height="50"/>
  <img src="https://github.com/juniorjrjl/juniorjrjl/blob/main/assets/logos/ame-digital.png" alt="AME Digital" width="50" height="50"/>
</p>
